    ...
    plotHR <- function (model , terms = 1 , se = TRUE , col.se = "#DEEBF7",
        ...
        quantiles <- quantile(data[,term.label] , probs = probs )
        ...
        #plot( data$fit ~ log(data[ , term.label]) , xlim = log(xlim) , ylim = ylim , xlab = xlab , ylab = ylab , main = main, type = "n" , axes = FALSE )
        # Remove rows where x or y values are <= 0
        data <- data[data[, term.label] > 0 & data$fit > 0, ]
        plot(
          data$fit ~ data[, term.label],
          xlim = c(1.5,5),
          ylim = c(0.1,20),
          xlab = xlab,
          ylab = ylab,
        ...
        y_ticks <- c(log(2), log(5), log(10), log(100), log(1000), log(10000), log(100000)) # 目盛りの位置
        axis(2, at = y_ticks, labels = as.character(y_ticks), las = 1) # y軸ラベルをカスタマイズ
